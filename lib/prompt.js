...
- 이미 자연스럽고 문제가 없는 문장은 그대로 둔다. 종결 표현을 다양하게 하려고 멀쩡한 문장을 덜 자연스럽게 바꾸지 않는다.
- 반복되는 연결어를 줄일 때 '이러한', '이를 통해', '결국', '결과적으로' 같은 다른 상투적 연결어로 바꾸지 않는다. 연결어 없이 이어지면 빼는 편을 우선한다.
- 수정 강도가 낮을수록 원문 문장 수와 순서를 유지한다.
